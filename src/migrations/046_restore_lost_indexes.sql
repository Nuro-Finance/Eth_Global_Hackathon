...
-- ── card_transactions: idempotency key for Issuer sync upsert ─────────────────
...
-- Non-unique. Speeds up "what Issuer transactions has user X seen" queries
...
-- ── issuer_webhook_events: X-Issuer-Webhook-Id dedup ────────────────────────────
-- Issuer retries deliveries on 5xx. Without this index, retried webhooks would
