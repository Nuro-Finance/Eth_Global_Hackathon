...
-- Sprint 2.4 — Real Visa spend data from Issuer
--
...
-- 1. Idempotency key — unique per Issuer transaction (webhook + sync pull converge here)
ALTER TABLE card_transactions
  ADD COLUMN IF NOT EXISTS issuer_transaction_id VARCHAR(100);

-- Partial unique index: allows NULL for legacy rows, enforces uniqueness for Issuer-sourced rows
...
-- 3. Raw Issuer merchant category — preserves what Issuer sent for future mapping improvements
ALTER TABLE card_transactions
...
-- 7. Webhook delivery dedup — Issuer may retry; X-Issuer-Webhook-Id makes each delivery unique
ALTER TABLE issuer_webhook_events
  ADD COLUMN IF NOT EXISTS issuer_delivery_id VARCHAR(100);
