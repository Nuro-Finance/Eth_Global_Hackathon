...
-- the Issuer issuer_user_id value, not the local users.id. This caused
...
-- puzzle: Gmail account (db01a59c) saw Chris's deposits
...
-- the Issuer value to the local users.id, mapping via
-- `users.issuer_user_id = OLD_VALUE OR users.issuer_user_id = OLD_VALUE`.
--
-- WHAT WE DO NOT DO
-- Do NOT touch the `address` column in deposit_addresses. On-chain
-- addresses are HD-derived from the Issuer UUID and must stay stable —
-- users who previously sent funds to an address need that address
-- to keep working. Only the user_id column value migrates.
--
-- Orphan rows (test fixtures + one failed Issuer session for 4422e442)
...
-- - src/nuro-routes.ts: saveDepositAddress(issuerUserId|issuerUserId, ...)
-- → saveDepositAddress(userId, ...) on every write site. HD seed
-- still uses issuerUserId (on-chain addresses unchanged).
...
SELECT 'deposit_addresses', da.user_id || '/' || da.chain, da.user_id,
       (SELECT u.id FROM users u
         WHERE u.issuer_user_id = da.user_id
            OR u.issuer_user_id    = da.user_id
         LIMIT 1)
