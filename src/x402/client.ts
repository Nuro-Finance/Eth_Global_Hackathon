...
//
//   agent.x402Fetch(url)
//     → enforceTxCap(HELM-105)         budget + tx-cap gate
...
// agentId. Mirrors src/bridge.ts pattern. Mythos gets a deterministic vault
// at sha256(PRIVATE_KEY + 'mythos'). Each user-attached agent uses their
...
export interface X402FetchInput {
  ...
  url: string
  /** Caller's agent attribution. 'mythos' / 'system' / userId. Drives both the
 ...
 *
 * Pre-flight chain:
 *   1. enforceTxCap (HELM-105) — agent's budget cap + global tx cap
...
export async function x402Fetch(
  ...
  // ── PRE-FLIGHT 1: HELM-105 tx-cap (budget-aware via getEffectiveUsdCap)
