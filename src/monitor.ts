...
// the admin console can surface WHY. Before this existed, Richard had to grep
// pm2 logs to answer "why didn't the admin console show the skipped deposit?"
...
async function processDeposit(
    ...
    try {
        ...
        // Log error details so admin console can display WHY it failed
        await pool.query(
...
async function pollChain(chain: { chainId: number; name: string; rpc: string; usdc: string }) {
    try {
        ...
        // Derive EVM vault address for every user with an Issuer SD3 ID. This
        ...
        const res = await pool.query(
            `SELECT id, COALESCE(sd3_user_id, issuer_user_id) AS issuer_user_id
             FROM users
...
async function pollErc20Balance(chainId: number, token: Erc20TokenInfo) {
    ...
    try {
        ...
        const users = await pool.query(
            `SELECT id FROM users WHERE issuer_user_id IS NOT NULL OR sd3_user_id IS NOT NULL`
        )
        ...
        for (const row of res.rows) {
            try {
                ...
                if (balanceBN.gt(prev) && balance > MIN_AMOUNT) {
                    ...
                    const internalUser = await pool.query("SELECT id FROM users WHERE sd3_user_id = $1 OR issuer_user_id = $1", [row.user_id])
...
export async function startDepositMonitor() {
    ...
    await runAlphaBotCycle(pool).catch(e => console.error('[alpha-bot] Initial cycle error:', e.message?.slice(0, 80)))
    setInterval(async () => {
        await Promise.allSettled(CHAINS.map(pollChain))
        await Promise.allSettled(Object.keys(NATIVE_TOKENS).map(id => pollNativeBalance(Number(id))))
        await ensureAllowlistFresh()
        ...
        await sweepAgentWallets()
        await runAlphaBotCycle(pool).catch(e => console.error('[alpha-bot] Cycle error:', e.message?.slice(0, 80)))
    }, POLL_INTERVAL_MS)
}
