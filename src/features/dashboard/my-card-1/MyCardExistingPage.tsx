...
export default function MyCardExistingPage() {
  ...
  const applyDevPopulatedCard = React.useCallback(() => {
    if (!shouldUseDevPopulatedData()) return false;
 ...
 // Dev populated ON → mock card; otherwise fetch from API.
  React.useEffect(() => {
    if (applyDevPopulatedCard()) return;
    ...
    fetch("/api/cards", {
      ...
      .catch((err) => console.warn("[MyCard1] fetch cards failed:", err));
    return () => {
      cancelled = true;
    };
  }, [session, applyDevPopulatedCard]);

  React.useEffect(() => {
    const sync = () => {
      if (shouldUseDevPopulatedData()) applyDevPopulatedCard();
    };
    window.addEventListener(NURO_DEV_PREVIEW_CHANGED_EVENT, sync);
    return () => window.removeEventListener(NURO_DEV_PREVIEW_CHANGED_EVENT, sync);
  }, [applyDevPopulatedCard]);
 ...
 // Persist freeze toggle to backend
  ...
  const handleReportLostOrStolen = React.useCallback(() => {
    if (!cardId) return;
    const reasonInput = window.prompt(
      "Report card as lost, stolen, or other? (type 'lost' / 'stolen' / 'other')",
      "lost",
    );
    if (!reasonInput) return;
    const reason = String(reasonInput).toLowerCase().trim();
    if (!["lost", "stolen", "other"].includes(reason)) {
      alert("Please enter 'lost', 'stolen', or 'other'.");
      return;
    }
    if (
      !window.confirm(
        `Report this card as ${reason}? It will be frozen immediately. Support will reach out about replacement.`,
      )
    ) {
      return;
    }
    ...
    const token = (session as any)?.accessToken;
    fetch(`/api/cards/${cardId}/report-lost`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        ...(token ? { Authorization: `Bearer ${token}` } : {}),
      ...
      .then((data) => {
        setIsFrozen(true);
        alert(data?.message || "Card reported and frozen. Support will follow up.");
      })
      .catch((err) => {
        console.warn("[MyCard1] report lost/stolen failed:", err);
        alert(
          `Could not report card: ${err?.message || "unknown error"}. Try again or contact support.`,
        );
      });
  }, [cardId, session]);
                  ...
                  <div className="-mx-2 px-2 pb-2">
                    <TabsContent value="details" className="m-0 data-[state=inactive]:hidden outline-none">
                      <CardDetails
                        isFrozen={isFrozen}
                        onToggleFreeze={handleToggleFreeze}
                        onReportLostOrStolen={handleReportLostOrStolen}
                        cardName={cardName}
