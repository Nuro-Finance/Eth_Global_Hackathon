...
/** Caret + H1 move as one unit (px); same duration for x and caret opacity. */
...
export function ReloadModal({ open, onOpenChange }: ReloadModalProps) {
  ...
  const handleOpenChange = useCallback(
    (next: boolean) => {
 // After progress (step 3) or success (step 4), closing the modal fires the header pill for QA / demo.
      if (!next && flowStep >= 3) {
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      >
        <div
          className="relative flex min-h-0 w-full min-w-0 flex-1 flex-col overflow-visible rounded-[44px] border !backdrop-blur-none"
          style={{
        ...
        >
          ...
          {/* Body + dots: 16px gap so primary button sits 16px above pagination. */}
