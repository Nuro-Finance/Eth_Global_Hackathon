 ...
 *
 * Pagination: uses Issuer's postedAfter + cursor per Integration Guide §5.3.
 */

import { Pool, PoolClient } from 'pg'
import { getIssuerTransactionsPage } from './issuers'
import { extractEventData, mapIssuerSpendToCardTx, CardTxInsert } from './issuer-mapping'
 ...
 * per skipped Issuer item explaining why. Used by the admin
...
export async function syncIssuerTransactions(
 ...
 // Cap the diagnostics list so a noisy Issuer response can't blow up memory.
 ...
 // Look up Issuer user id + since-cursor
  const userRes = await db.query(
    `SELECT id, issuer_user_id, last_tx_synced_at
  ...
  const issuerUserId: string | null = user.issuer_user_id
  if (!issuerUserId) {
    result.error = 'no_issuer_user_id'
    return result
  }

 // S35 M11 Day-3 evening: 24h lookback buffer. Earlier today (post-incident
 // fix) noticed several Visa spends synced but Krispy Kreme didn't.
 // Symptom matches the watermark-too-tight failure mode: Issuer posts the
 ...
 // Issuer-claimed postedAt timestamp.
  ...
  try {
    do {
      const page = await getIssuerTransactionsPage(issuerUserId, {
        postedAfter,
      ...
      for (const evt of page.items) {
        ...
        const issuerTxType = (extracted.data?.type || '').toLowerCase()
 // Skip collateral (monitor owns the deposit path)
        ...
        const issuerCardId: string | undefined = extracted.data?.spend?.cardId
        ...
        if (issuerCardId) {
          ...
          if (!cardRes.rows[0]) {
            await reportWarning(
              ...
              'sync_unknown_card',
              issuerCardId,
              `Sync found Issuer card id not linked to user ${userId}`
            )
            result.skipped++
            recordSkip({ reason: 'card_not_linked', raw: truncate(evt), detail: `issuerCardId=${issuerCardId}` })
            continue
          }
 ...
 // demo-stack cards.
          ...
          result.skipped++
          recordSkip({ reason: 'no_issuer_card_id', raw: truncate(evt), detail: `type=${issuerTxType}` })
          continue
        }

 // TS narrowing: every reachable path above either assigned dbCardId
 // or `continue`'d.
        const row = mapIssuerSpendToCardTx(extracted.data, dbCardId as string, userId)
...
/**
 * Some Issuer responses for /transactions return bare transaction objects (no envelope).
