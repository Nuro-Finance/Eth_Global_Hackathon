/**
 * ─── Issuer → CARD_TRANSACTIONS MAPPING ─────────────────────────────────────────
 *
 * Pure functions mapping Issuer webhook/API payloads into our card_transactions
 * row shape. No DB calls; call sites handle upsert.
 *
 * Issuer envelope shapes seen in production code:
...
export function mapIssuerMerchantCategory(raw: string | null | undefined): string {
  if (!raw) return 'other'
  const s = String(raw).trim()
  if (!s) return 'other'

 // Case 1: Issuer sent a numeric MCC
  if (/^\d{3,4}$/.test(s)) {
...
export function mapIssuerTxStatus(status: string | null | undefined): 'pending' | 'completed' | 'failed' {
...
export interface IssuerSpendPayload {
  id?: string
  amount?: number            // cents per Issuer Integration Guide
  currency?: string
  merchantName?: string
...
export interface IssuerTransactionData {
  id?: string
  type?: 'spend' | 'fee' | 'payment' | 'collateral' | string
  spend?: IssuerSpendPayload
  fee?: IssuerSpendPayload
  payment?: IssuerSpendPayload
  collateral?: any
  [key: string]: any
}
...
/**
 * Map an Issuer transaction `data` object to our card_transactions row shape.
 * `dbCardId` is our internal card PK (already looked up from cards.issuer_card_id).
 */
export function mapIssuerSpendToCardTx(
  data: IssuerTransactionData,
  dbCardId: string,
  dbUserId: string
): CardTxInsert {
  const issuerTxType = (data.type || 'spend').toLowerCase()
  const inner: IssuerSpendPayload = data.spend || data.fee || data.payment || (data as unknown as IssuerSpendPayload)

  const amountCents = typeof inner.amount === 'number' ? inner.amount : 0
  const amountUsd = amountCents / 100
  const signedAmount = issuerTxType === 'payment' ? Math.abs(amountUsd) : -Math.abs(amountUsd)

  const txType: CardTxInsert['type'] =
    issuerTxType === 'payment' ? 'deposit' : 'purchase'
  const transactionType =
    issuerTxType === 'fee' ? 'issuer_fee' :
    issuerTxType === 'payment' ? 'issuer_payment' :
    'visa_spend'
 ...
 // rows for ~14h). Cheap insurance against the next time Issuer returns
 ...
 // direction. We trust issuerTxType - the issuer is authoritative.
  ...
  return {
    cardId: dbCardId,
    userId: dbUserId,
    name: truncate(inner.merchantName, 255) || (issuerTxType === 'fee' ? 'Issuer fee' : issuerTxType === 'payment' ? 'Card payment' : 'Purchase'),
    type: txType,
    amount: signedAmount,
    isIncoming,
    category: mapIssuerMerchantCategory(inner.merchantCategory),
    status: mapIssuerTxStatus(inner.status),
...
/**
 * Normalize the Issuer webhook envelope across the two observed shapes.
