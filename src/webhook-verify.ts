 ...
 * ops tools has a complete audit trail.
 */
...
export function verifyRawBodyHmac(
 ...
 // Allow optional "sha256=" prefix (Stripe / some Issuer variants)
...
/**
 * Express middleware factory for Issuer Issuer webhooks.
 *
...
export function createIssuerWebhookVerifier(
  ...
  return async (req, res, next) => {
    const signature = (req.header('X-Issuer-Signature') || req.header('x-issuer-signature') || '') as string
 ...
 // Production path — verify signature
    if (!signature) {
      ...
      await reportError(
        ...
        'webhook_missing_sig',
        config.endpoint,
        'Missing X-Issuer-Signature header'
      )
