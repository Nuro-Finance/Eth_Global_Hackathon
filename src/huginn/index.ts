...
//
// S31 H2. Per Mythos's "should I?" gate + Marathon 8 / Corvus dissent
...
// resolves. Builds Huginn's tier over time → counsel-weight in Mythos's
// decision pipeline.
...
export interface CounselInput {
  /** Who's asking. e.g. 'mythos', 'execution-dispatch'. */
...
// Rule 3: Helm Gjallarhorn state.
const ruleGjallarhornState: Rule = async ({ db, input }) => {
  try {
    const r = await db.query(
...
// Rule 4: tx-cap proximity — action value vs. effective HELM-105 cap.
...
function summarizeReasoning(verdict: CounselVerdict, signals: CounselSignal[]): string {
  if (signals.length === 0) {
    return 'No risk signals fired. Proposer is in good standing across budget, reputation, Helm state, and recent track record.'
  }
...
function recommendedAlternative(verdict: CounselVerdict, signals: CounselSignal[]): string | undefined {
  ...
  if (codes.has('cap-exceeded') || codes.has('cap-proximity')) {
    return 'Split into smaller transactions OR raise HELM-105 cap via admin UI if intentional.'
  }
  if (codes.has('heimdall-paused') || codes.has('heimdall-quarantined')) {
    return 'Resolve the Helm state first (admin override required); do not bypass.'
  }
...
export async function counsel(db: Pool, input: CounselInput): Promise<CounselResult> {
  ...
  try {
    const { recordPrediction } = await import('../reputation')
    const { predictionId: pid } = await recordPrediction(db, {
      agentId: 'huginn',
      type: 'counsel-outcome',
      subject: input.actionSubject,
      prediction: {
        verdict,
        forecastedOutcome: verdict === 'endorse' ? 'succeeds' : verdict === 'caution' ? 'succeeds-with-friction' : 'fails-or-blocked',
      ...
      confidence,
      reasoning,
      horizonDays: 1, // outcome usually resolves within 24h for tx-class actions
    })
    predictionId = pid
  } catch (err: any) {
    console.warn(`[huginn] prediction record failed: ${err?.message?.slice(0, 100)}`)
  }

  // Best-effort: log a HELM-CTRL event so the admin UI shows Huginn's
  // counsel alongside other Helm activity.
  try {
 ...
 *   - (future) gjallarhorn-transition — proposes operational responses
 *
 * The subscriber writes a counsel message back to the bus on the
 * `huginn-counsel` topic. Subscribers to THAT topic (Mythos, admin UI)
 * see Huginn's reactions in real time.
 */
export async function bootstrapHuginnSubscriptions(db: Pool): Promise<void> {
  try {
    const { subscribe, publish, read } = await import('../agent-bus')
    await subscribe(db, 'huginn', 'external-doc-drift:breaking')
    await subscribe(db, 'huginn', 'agent-budget-low')
    // Also pull any messages already pending so we don't miss anything
    // that landed during boot. We process them synchronously here once;
    // subsequent messages flow through the cron poller below. The
    // undeliveredOnly flag guarantees at-most-once react semantics across
    // boot + cron — without it, every poll re-fires counsel for the
    // same source message.
    const pending = await read(db, 'huginn', { limit: 25, markDeliveredOnFetch: true, undeliveredOnly: true })
    for (const msg of pending) {
      await reactToBusMessage(db, msg, publish)
    }
  } catch (err: any) {
    console.warn(`[huginn] bootstrap failed: ${err?.message?.slice(0, 100)}`)
  }
}

/**
 * Cron pass: pull any new bus messages addressed to Huginn (or on
 * subscribed topics), react with counsel, publish.
 */
export async function runHuginnPollCycle(db: Pool): Promise<{ messagesProcessed: number }> {
  let processed = 0
  try {
    const { read, publish } = await import('../agent-bus')
    const messages = await read(db, 'huginn', { limit: 25, markDeliveredOnFetch: true, undeliveredOnly: true })
    for (const msg of messages) {
      try {
        await reactToBusMessage(db, msg, publish)
        processed++
      } catch (err: any) {
        console.warn(`[huginn] react failed for ${msg.id}: ${err?.message?.slice(0, 100)}`)
      }
    }
  } catch (err: any) {
    console.warn(`[huginn] poll cycle failed: ${err?.message?.slice(0, 100)}`)
  }
  return { messagesProcessed: processed }
}

async function reactToBusMessage(
  db: Pool,
  ...
  if (msg.topic === 'external-doc-drift:breaking') {
    ...
    // operational pause + review. Publishes counsel back so Mythos /
    // admin UI sees it.
  ...
  if (msg.topic === 'agent-budget-low') {
    ...
    // Huginn publishes counsel + fires Telegram so Richard can refill or
