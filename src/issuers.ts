...
/**
 * SD3's published RSA public key for card-secret reveals (production — only
 * environment that exists). 1024-bit RSA, used with RSA-OAEP / SHA-1 padding.
 *
 * Sourced directly from SD3 docs (Owen, 2026-05-08). Full integration spec
 * lives in `.claude/skills/sd3-card-secrets/SKILL.md`. Do NOT change padding
...
// Helm HELM-101 — observe outbound traffic to SD3 hosts. Observe-only
// unless HELM_EGRESS_ENFORCE=on. Both rocket.sd3.gg and CONFIG.ISSUER_API_BASE
// (rocket.sd3.gg) are on the default allowlist, so legitimate traffic
// won't trip events. Lazy require keeps this safe in test/standalone runs
// where heimdall isn't booted.
try {
...
// ─── SD3 CARD DEBIT / CREDIT (Buy 1 — Session 28) ────────────────────────────
// Conventional REST endpoints; actual SD3 path may differ — Owen conversation
// pending. The helper calls the most likely path; if SD3 uses a different
...
//   404 → endpoint doesn't exist on SD3 side (Buy 1 impossible until Owen confirms)
//   400 → bad params (amount, etc.)
//   402 → insufficient card balance
//   500 → SD3 internal error
...
export async function debitCard(
  ...
  const response = await issuingClient.post<DebitResult>(
    `/cards/${issuerCardId}/debit`,
    {
      ...
      // SD3 should recognize this key and return the original response on retry.
      idempotencyKey,
 ...
 * Used for reconciliation when Buy 1 mid-flight fails (SD3 debit succeeded
...
//
// Supports cursor pagination and since-timestamp filtering per SD3 Integration
...
export async function getIssuerTransactionsPage(
    ...
    try {
        ...
        const data = response.data || {}
        // SD3 responses observed as either { transactions: [], cursor } or { items: [], cursor } or plain array
...
export async function getIssuerCardNumber(issuerCardId: string): Promise<{ cardNumber: string | null; expiryDate: string | null; cvv: string | null } | null> {
    try {
        ...
        // SD3's metadata endpoint (`GET /cards/:id`) deliberately does NOT return
        ...
        // ships (Owen's RSA + AES-GCM protocol).
...
/**
 * Generate a fresh SessionId for SD3's RSA-OAEP-gated reveal endpoints
 * (`/cards/:id/secrets`, `/cards/:id/pin`).
 *
 * Protocol (per SD3 docs — see `.claude/skills/sd3-card-secrets/SKILL.md`):
 ...
 *   3. RSA-OAEP / SHA-1 encrypt those base64 bytes with SD3's public key
 ...
 * AES-128-GCM-encrypted (Owen mentioned this shape once though the official
...
function generateSessionId(): { secretKey: string; sessionId: string } {
    const pub = createPublicKey(SD3_PUBLIC_KEY_PEM)
    ...
    // SD3's server reconstructs after RSA-decrypt → AES-GCM fails the auth
...
/**
 * AES-128-GCM decrypt a single SD3-encrypted secret field.
 *
 * Wire format (per Owen, verified against live API 2026-05-08):
...
interface SecretsApiResponse {
    ...
    // Plaintext fallback shape some SD3 docs reference — we accept either
...
/**
 * Fetch the full PAN / CVV from SD3's encrypted reveal endpoint.
...
export async function getIssuerCardSecrets(issuerCardId: string): Promise<IssuerCardSecrets | null> {
    ...
    try {
        ...
        // Plaintext-shape fallback (what SD3's official docs claim)
