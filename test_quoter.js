const {ethers} = require('ethers')
const p = new ethers.providers.JsonRpcProvider('https://rpc.hyperliquid.xyz/evm')

const QUOTER_V2 = '0x03A918028f22D9E1473B7959C927AD7425A45C7C'
const WHYPE = '0x5555555555555555555555555555555555555555'
const USDC  = '0xbe6727b535545c67d5caa73dea19b7c5cc3d9b9e'

// QuoterV2 takes a struct for quoteExactInputSingle
const ABI = [
  'function factory() view returns (address)',
  'function WETH9() view returns (address)',
  'function quoteExactInputSingle((address tokenIn, address tokenOut, uint256 amountIn, uint24 fee, uint160 sqrtPriceLimitX96) params) external returns (uint256 amountOut, uint160 sqrtPriceX96After, uint32 initializedTicksCrossed, uint256 gasEstimate)'
]

const q = new ethers.Contract(QUOTER_V2, ABI, p)

;(async () => {
  const factory = await q.factory()
  const weth = await q.WETH9()
  console.log('factory():', factory)
  console.log('WETH9():', weth)

  // Quote 1 WHYPE -> USDC at 0.3% fee
  const result = await q.callStatic.quoteExactInputSingle({
    tokenIn: WHYPE,
    tokenOut: USDC,
    amountIn: ethers.utils.parseEther('1'),
    fee: 3000,
    sqrtPriceLimitX96: 0
  })
  console.log('1 WHYPE =>', ethers.utils.formatUnits(result.amountOut, 6), 'USDC')
  console.log('gasEstimate:', result.gasEstimate.toString())
})()
