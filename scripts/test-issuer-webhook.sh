...
fire() {
  ...
  status=$(curl -sS -o /tmp/issuer-webhook-resp.json -w "%{http_code}" \
    ...
    -H "SD3-Signature: $sig" \
    -H "SD3-Webhook-Id: $webhook_id" \
    -H "SD3-Partner-API: test-partner" \
    --data "$body")
...
spend_body='{
  ...
  "data": {
    ...
    "spend": {
      ...
      "status": "completed",
      "cardId": "card_test_sd3_001",
      "userId": "user_test_sd3_001",
...
card_body='{
  ...
  "action": "created",
  "data": {
    "id": "card_test_sd3_999",
    "cardId": "card_test_sd3_999",
    "userId": "user_test_sd3_001",
    "type": "virtual",
    "status": "active"
  },
...
echo ""
echo "  SELECT sd3_webhook_id, event_type, processed, process_result FROM issuer_webhook_events"
