...
WITH expected_schema(table_name, column_name, expected_type) AS (VALUES
  ...
  ('users',               'issuer_user_id',         'text'),              -- VERIFIED: text on prod (not VARCHAR despite some migration declarations)
...
WITH expected_schema(table_name, column_name, expected_type) AS (VALUES
  ...
  ('agent_profit_sweeps','status','character varying'),
  ('users','id','character varying'),('users','payout_destination','character varying'),('users','issuer_user_id','text'),
